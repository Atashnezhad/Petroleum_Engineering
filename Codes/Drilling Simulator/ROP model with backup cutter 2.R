...
EXPR_new <- 0.03 #back up cutter exposed when both cutters are new
...
WOB <- read.csv("/cloud/project/WOB.csv", header = T); WOB <-2*WOB
WOC <- WOB/NOC
...
RPM <- read.csv("/cloud/project/RPM.csv", header = T); RPM <- 1*RPM
#----------------------------CCS Column
...
CCS <- read.csv("/cloud/project/CCS.csv", header = T); CCS <- 1*CCS
#----------------------------Tf Column
...
wear_cof_PDC <- 20000000 # (ft.Lbf)/inch3 of pure PDC material
...
Diff_AwBeneathCutter_T_NeedToCovered <- 0 -NeedToCovered
colnames(Diff_AwBeneathCutter_T_NeedToCovered) <- 'Diff_AwBeneathCutter_T_NeedToCovered'


#BackUp cutter is engaged with the formation or not
BU_cutter_engage <- matrix(0.5,Num_of_Data,1)
colnames(BU_cutter_engage) <- "1=YES 0=NO"
